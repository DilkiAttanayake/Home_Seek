...
function nav() {
    ...
    const handleSearch = () => {
        ...
        const results = propertiesData.properties.filter(property =>
            property.type.toLowerCase().includes(searchType.toLowerCase()) // Filter properties based on search type
        );
        setFilteredResults(results); // Update filtered results state
    };
            ...
            <nav className="navbar navbar-light" style={{ backgroundColor: '#e3f2fd', border: '2px solid #90caf9' }}>
                <div className="container-fluid"> {/* Full width container */}
                    ...
                    <div className="navbar-brand">
                        <img src={Logo} onClick={() => window.location.href = '/'} alt="Logo" style={{ width: '100px', cursor:'pointer' }} />
                    </div>
